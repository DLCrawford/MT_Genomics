...
#   produced by BSUB_Slim_BAM_mt.sh. Run that array first.
#
# Submit:
#   bsub < jobs/BSUB_Slim_BAM_mt.sh        # produces the slim BAMs
#   # then, AFTER the slim array completes:
#   bsub < jobs/05d2_persample_call.sh     # this script
#   # then, AFTER the per-sample array completes:
#   bsub < jobs/05e2_merge_persample.sh    # merge into population VCF
###############################################################################
...
# We translate each entry to the slim mt-only BAM produced by BSUB_Slim_BAM_mt.sh:
#   bams/10_0_MT.bam   →   MT_only_bams/10_0_MT_only.bam
# Slim BAMs are ~50-100 MB each (vs 15-19 GB), so mpileup is minutes not hours.
...
BAM="${SLIM_BAMS_DIR}/${SAMPLE}_MT_only.bam"

if [[ ! -s "$BAM" ]]; then
    echo "ERROR: slim BAM not found or empty: ${BAM}" >&2
    echo "       Did BSUB_Slim_BAM_mt.sh complete successfully for this sample?" >&2
    exit 1
fi
if [[ ! -s "${BAM}.bai" ]]; then
    echo "ERROR: slim BAM index not found: ${BAM}.bai" >&2
    exit 1
fi
