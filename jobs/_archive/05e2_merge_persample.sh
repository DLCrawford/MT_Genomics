...
#
# Why this exists:
#   05d2 emits one normalized VCF per sample under ${VCF_DIR}/persample/,
#   built from the slim mt-only BAMs (BSUB_Slim_BAM_mt.sh). This script
#   unions them with `bcftools merge -m none` (one row per POS+ALT),
#   reheaders to strip the trailing "_0" from sample names, and writes stats
#   + a self-documenting manifest.
#
...
#
# Pre-flight:
#   Run only AFTER 05d2's array has completed and all 143 *_norm.vcf.gz
#   files exist. A simple check is included below.
#
# Submit:
#   bsub < jobs/BSUB_Slim_BAM_mt.sh        # if not already done
#   bsub < jobs/05d2_persample_call.sh     # if not already done
#   bsub < jobs/05e2_merge_persample.sh    # this script
###############################################################################
...
# === Manifest BEFORE the long step ===
{
    ...
    echo "                  per-sample call (05d2) → bcftools merge -m none (05e2)"
    echo "rationale:        replicate historical merged_144 recipe; recover the"
    echo "                  ~1133-SNP signal that joint -mv suppresses on mtDNA"
    echo "                  with a divergent reference. Slim BAMs cut mpileup"
    echo "                  walltime ~100x vs the 15-19 GB WGS-aligned BAMs."
    echo
    echo "REF:              ${REF}"
    echo "SLIM_BAMS_DIR:    ${PROJECT_ROOT}/MT_only_bams"
    ...
    echo
    echo "Slim-BAM command (in BSUB_Slim_BAM_mt.sh):"
    echo "  samtools view -@ 4 -b -F 2308 -o \${SAMPLE}_MT_only.bam \\"
    echo "    \$IN_BAM NC_012312.1"
    echo "  samtools index -@ 4 \${SAMPLE}_MT_only.bam"
    echo
    echo "Per-sample command (in 05d2):"
    echo "  bcftools mpileup -f \$REF \$SLIM_BAM -a AD,DP --max-depth 10000 -Ou \\"
...
# === Rename samples ===
# The slim BAMs (BSUB_Slim_BAM_mt.sh output) carry no @RG SM: tag, so
# `bcftools mpileup` in 05d2 falls back to the BAM filename. Per-sample
# VCFs therefore inherit names like
#   /projectnb/dcrawford/MT_Genomics2/MT_only_bams/10_0_MT_only.bam
# Convert to "10_MT", "102_MT", etc: drop the directory prefix and swap
# the "_0_MT_only.bam" suffix for "_MT".
echo "[$(date)] Reheadering: stripping path + renaming to *_MT..."
bcftools reheader \
